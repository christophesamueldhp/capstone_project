...
# pilih1_chart= st.selectbox('Masukkan daerah yang dipilih',('Kepulauan Seribu','Jakarta Pusat','Jakarta Utara','Jakarta Selaran','Jakarta Timur','Jakarta Barat','DKI Jakarta'))
# if pilih1_chart=='Kepulauan Seribu':
#     pen_df66 = df66[['tahun', 'jumlah_penduduk']].set_index('tahun').set_index('tahun').resample('Y').mean()
#     st.line_chart(pen_df66)
# elif pilih1_chart=='Jakarta Pusat':
#     pen_df11 = df11[['tahun', 'jumlah_penduduk']].set_index('tahun').set_index('tahun')
#     st.line_chart(pen_df11)
# elif pilih1_chart=='Jakarta Utara':
#     pen_df55 = df55[['tahun', 'jumlah_penduduk']].set_index('tahun').set_index('tahun')
#     st.line_chart(pen_df55)
# elif pilih1_chart=='Jakarta Selatan':
#     pen_df44 = df44[['tahun', 'jumlah_penduduk']].set_index('tahun').set_index('tahun')
#     st.line_chart(pen_df44)
# elif pilih1_chart=='Jakarta Timur':
#     pen_df33 = df33[['tahun', 'jumlah_penduduk']].set_index('tahun').set_index('tahun')
#     st.line_chart(pen_df33)
# elif pilih1_chart=='Jakarta Barat':
#     pen_df22 = df22[['tahun', 'jumlah_penduduk']].set_index('tahun').set_index('tahun')
...
st.subheader('Hubungan antara Indeks Standar Pencemaran Udara (ISPU) dan Kepadatan Penduduk DKI JAKARTA')

st.markdown('Berdasarkan Uji Korelasi Pearson, nilai yang didapat adalah 0.443262. Hal tersebut tidak bisa menunjukkan korelasi antara kepadatan penduduk dengan tingkat pencemaran udara')
...
st.subheader('Hubungan antara Indeks Standar Pencemaran Udara (ISPU) dan Jumlah Sampah di DKI JAKARTA')

st.markdown('Berdasarkan Uji Korelasi Pearson, nilai yang didapat adalah -0.232736. Hal tersebut tidak bisa menunjukkan korelasi antara kepadatan penduduk dengan tingkat pencemaran udara')
st.markdown('Grafik di bawah adalah grafik kepadatan penduduk dan indeks standar pencemaran udara (ISPU) di DKI Jakarta yang sudah dinormalisasi.')
sampah_polusi= pd.read_csv('sampah_polusi.csv')
...
st.markdown('Kedua hubungan di atas menunjukkan Indeks Standar Pencemaran Udara (ISPU) DKI Jakarta tidak berhubungan dengan kepadatan penduduk dan jumlah sampah. Perlu dicari kembali faktor-faktor penyebab terjadinya meningkatnya pencemaran udara')
st.markdown('Akan tetapi, terdapat kemungkinan kedua hubungan tersebut berkorelasi. Hal tersebut dikarenakan jumlah data yang kurang lengkap dan tidak mewakili DKI Jakarta.')
